...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $topPerformers = Student::with(['evaluations'])
            ->whereHas('group', fn($q) => $q->where('user_id', $user->id))
            ->get()
            ->map(function ($student) {
                $stats = $student->getCurrentStats();
                // Evolução = Atual - Inicial (Negativo significa que emagreceu)
                $student->evolution_total = $stats->weight - $student->weight;
                $student->current_weight = $stats->weight;
                return $student;
            })
            ->sortBy('evolution_total') // Ordena por quem mais perdeu peso
            ->take(5);
